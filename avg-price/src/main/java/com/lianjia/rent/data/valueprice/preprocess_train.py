...
def preprocess():
    df=pd.read_csv("bj_train.csv",sep="\t")
    #df=pd.read_csv("cd_train.csv",sep="\t")
    ...
    bt_df=df[['floor_level','hdic_resblock_id','decoration']]
    dummies_hdic_resblock_id = pd.get_dummies(bt_df['hdic_resblock_id'], prefix= 'hdic_resblock_id')
    dummies_decoration = pd.get_dummies(bt_df['decoration'], prefix= 'decoration')
    bt_df = pd.concat([bt_df,dummies_decoration,dummies_hdic_resblock_id], axis=1)
    bt_df.drop(['decoration','hdic_resblock_id'], axis=1, inplace=True)
    ye=bt_df[bt_df['floor_level']==0]
    no=bt_df[bt_df['floor_level']!=0]
    ye_ma=ye.as_matrix()
    no_ma=no.as_matrix()
    y=no_ma[:,0]
    X=no_ma[:,1:]
    ...
    clf.fit(X,y)
    pred=clf.predict(non)
    df.loc[ (bt_df['floor_level']==0), 'floor_level' ] = pred
    ...
    df.drop(['building_finish_year','price_listing'],axis=1,inplace=True)
